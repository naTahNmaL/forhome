...
export class JourneyService {
  ...
  userId: any;
  constructor(private _router: Router, private _appAuthService: AppAuthService, private _httpClient: HttpClient) {
    this.userinfo = _appAuthService.user.subscribe(
      user => {
        this.userId = user?.id;
        this.init();
    })
    ...
    // for( const journey of journey)
  }

  private async init() {
    await this.getUserJourneys();
  }
  async getUserJourneys() {
    if (this.userId != null) {
      try {
        const response = await this._httpClient.get('/userinfo?id=' + this.userId).toPromise();
